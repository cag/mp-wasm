...
EXPORTED_MPFR_FUNCTIONS := $(shell grep -o '^__MPFR_DECLSPEC.* [\(\)]' ${MPFR_DIR}/src/mpfr.h | grep -Eo 'mpfr_\w+ +[\(\)]' | cut -d ' ' -f1 | grep -vwE 'mpfr_([gs]et_(decimal64|float128|[su]j\w*)|(inp|out)_str|[vasfn]*printf|fpif_(ex|im)port)' | ${POST_PROCESS_LIST})
...
libs-build.js: ${GMP_LIB} ${MPFR_LIB} ${MPC_LIB}
	emcc ${OPTIMIZATION_FLAG} ${GMP_LIB} ${MPFR_LIB} ${MPC_LIB} -o libs-build.js -s 'EXPORTED_FUNCTIONS=${EXPORTED_FUNCTIONS}'
